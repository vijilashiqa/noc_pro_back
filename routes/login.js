    ...
    return new Promise(async (resolve, reject) => {
        ...
        if (conn) {
            ...
            try {
                ...
                WHERE loginid = '${data.loginid}' AND pwd='${data.pwd}' And urole=${data.urole}
                `
                    ...
                    if (result[0].length == 1) {
                        ...
                        try {
                            ...
                            token = await jwt.sign({
                                id: userDet.id, loginid: userDet.loginid,fname:userDet.fname, email: userDet.email, mobile: userDet.mobile,
                                urole:userDet.urole,session_id: session_id,
                            },
                                privateKey, { algorithm: 'HS512', expiresIn: tokenExpireTime });
                                refresh_token = await jwt.sign({ id: userDet.id, loginid: userDet.loginid, session_id: session_id ,urole:userDet.urole},
